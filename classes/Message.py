...
class Message():
    ...
    def split_into_packets(self, net, dest):
        ...
        for i, f in enumerate(fragments):
            ...
            tmp_pkt = Packet.new(conf=self.conf, net=net, payload=f, sender=self.real_sender, dest=dest, msg_id=self.id, type="REAL", order=i+1, num=num_fragments, message=self)
            pkts.append(tmp_pkt)
            self.reconstruct.add(tmp_pkt.packet_id)
    ...
    def register_received_pkt(self, new_pkt):
        ...
        if self.complete_receiving: #Same packet may have been retransmitted.
            return
        elif new_pkt.msg_id == self.id and new_pkt.packet_id in self.reconstruct:
            self.reconstruct.remove(new_pkt.packet_id)
